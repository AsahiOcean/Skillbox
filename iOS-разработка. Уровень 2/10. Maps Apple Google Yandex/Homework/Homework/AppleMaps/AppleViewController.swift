...
class AppleViewController: UIViewController, CLLocationManagerDelegate {
    ...
    @IBAction func findMeAction(_ sender: UIButton) {
        if mapView.showsUserLocation == false {
            ...
            self.findMe.backgroundColor = .systemBlue
        } else {
            ...
            alert.addAction(UIAlertAction(title: "Да (Yes)", style: .default, handler: { action in
                ...
                self.findMe.backgroundColor = .lightGray
                
                self.mapView.centerToLocation(CLLocation(latitude: self.POI.center()?.latitude ?? 0, longitude: self.POI.center()?.longitude ?? 0), regionRadius: 2000)
            }))
    ...
    override func viewDidLoad() {
        ...
        mapView.centerToLocation(CLLocation(latitude: self.POI.center()?.latitude ?? 0, longitude: self.POI.center()?.longitude ?? 0), regionRadius: 2000)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            print("POI isEmpty?: \(self.POI.isEmpty)")
        }
    }
    
    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        self.mapView.removeAnnotations(POI)
    }
