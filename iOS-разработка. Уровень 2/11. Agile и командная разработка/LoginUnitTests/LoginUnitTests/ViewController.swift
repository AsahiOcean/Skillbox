...
class ViewController: UIViewController {
    ...
    @IBAction func loginEdit(_ sender: UITextField) {
        ...
        // guard "разрешит" дальнейшее выполнение кода, при условии sender.text != nil
    ...
    func loginButtonEnabled() {
        // .text! - т.к. guard в каждом из поле обрабатывает nil
        let loginNoNil = loginTextField.text!.count > 0
        let passNoNil = passTextField.text!.count > 0
        
        if loginNoNil && passNoNil {
            message.text?.removeAll()
